...
void HotelsHandler::handleRequest(HTTPServerRequest& request, HTTPServerResponse& response) {
    ...
    if (request.getMethod() == HTTPRequest::HTTP_POST) {
        ...
        std::string clientId = std::to_string(userId);

        if (!RateLimiter::instance().isAllowed(clientId, "/api/hotels/create", 100, 3600)) {
            response.add("X-RateLimit-Limit", "100");
            response.add("X-RateLimit-Remaining", std::to_string(
                RateLimiter::instance().getRemainingRequests(clientId, "/api/hotels/create", 100, 3600)
            ));
            response.add("X-RateLimit-Reset", std::to_string(
                RateLimiter::instance().getResetTime(clientId, "/api/hotels/create", 3600)
            ));
            sendError(response, HTTPResponse::HTTP_TOO_MANY_REQUESTS, "Rate limit exceeded");
            return;
        }
        ...
        try {
            ...
            // Чистим кеш
            RedisCache::instance().del("hotels:city:" + city);
            RedisCache::instance().del("hotels:all");
        ...
        if (!city.empty()) {
            cacheKey = "hotels:city:" + city;
            cachedData = RedisCache::instance().get(cacheKey);
        } else {
            cacheKey = "hotels:all";
            cachedData = RedisCache::instance().get(cacheKey);
        }
        ...
        RedisCache::instance().set(cacheKey, serialized, 3600);
...
void BookingsHandler::handleRequest(HTTPServerRequest& request, HTTPServerResponse& response) {
    ...
    if (request.getMethod() == HTTPRequest::HTTP_POST) {
        ...
        if (!RateLimiter::instance().isAllowed(clientId, "/api/bookings/create", 10, 60)) {
            response.add("X-RateLimit-Limit", "10");
            response.add("X-RateLimit-Remaining", std::to_string(
                RateLimiter::instance().getRemainingRequests(clientId, "/api/bookings/create", 10, 60)
            ));
            response.add("X-RateLimit-Reset", std::to_string(
                RateLimiter::instance().getResetTime(clientId, "/api/bookings/create", 60)
            ));
            sendError(response, HTTPResponse::HTTP_TOO_MANY_REQUESTS, "Rate limit exceeded");
            return;
        }
        ...
        try {
            ...
            // Очистка кеша по юзеру
            RedisCache::instance().del("bookings:user:" + std::to_string(userId));
        ...
        // кеш
        std::string cacheKey = "bookings:user:" + std::to_string(userId);
        std::string cachedData = RedisCache::instance().get(cacheKey);
        ...
        Poco::JSON::Stringifier::stringify(arr, ss);
        std::string serialized = ss.str();
        RedisCache::instance().set(cacheKey, serialized, 1800);
